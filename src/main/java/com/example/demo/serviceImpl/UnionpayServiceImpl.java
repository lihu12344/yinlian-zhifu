...
public class UnionpayServiceImpl implements UnionpayService {
    ...
    private String txnTime=DemoBase.getCurrentTime();
    ...
    public String pay(String orderId,String txnAmt) {
        ...
        requestData.put("bizType", "000301");           			  //业务类型，B2C网关支付，手机wap支付
        ...
        requestData.put("encryptCertId",AcpService.getEncryptCertId());
        ...
        return AcpService.createAutoFormHtml(frontRequestUrl, submitFromData,DemoBase.encoding);
    }
    ...
    @Override
    public Map<String,String> rollBack(String orderId) {
        Map<String, String> data = new HashMap<>();
    ...
    public Map<String,String> cancel(String orderId,String queryId) {
        ...
        data.put("txnAmt", "30000");                       //【撤销金额】，消费撤销时必须和原消费金额相同
    ...
    public Map<String,String> refund(String orderId,String queryId) {
        ...
        data.put("txnTime", DemoBase.getCurrentTime());      //订单发送时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
        data.put("currencyCode", "156");                     //交易币种（境内商户一般是156 人民币）
        data.put("txnAmt", "10000");                          //****退货金额，单位分，不要带小数点。退货金额小于等于原消费金额，当小于的时候可以多次退货至退货累计金额等于原消费金额
    ...
    @Override
    public String downloadBill(String settleDate) {
        Map<String, String> data = new HashMap<>();

        /***银联全渠道系统，产品参数，除了encoding自行选择外其他不需修改***/
        data.put("version", SDKConstants.VERSION_5_1_0);               //版本号 全渠道默认值
        data.put("encoding", SDKConstants.UTF_8_ENCODING);             //字符集编码 可以使用UTF-8,GBK两种方式
        data.put("signMethod", SDKConfig.getConfig().getSignMethod()); //签名方法
                ...
                if("00".equals(respCode)){
                    ...
                    for(String file : fileList){
                        if(file.contains("ZM_")){
                            fileContentDispaly = DemoBase.getFileContentTable(DemoBase.parseZMFile(file),file);
                        }else if(file.contains("ZME_")){
                            fileContentDispaly= DemoBase.getFileContentTable(DemoBase.parseZMEFile(file),file);
                        }
                    }
                    //TODO
                }else{
